...
export default function FaturaDetalhe() {
  ...
  const handleConfirmar = async (tx: Transacao) => {
    try {
      ...
      toast.success(t("cardsConfirmed"))
    } catch {
  ...
  return (
    <div style={{ padding: '20px 20px 100px', maxWidth: 720, margin: '0 auto', fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          <ArrowLeft size={15} /> {t("cardsAllCards")}
        </button>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: 'var(--of-text-muted)', marginBottom: 8 }}>
              ...
              {cartao.dia_vencimento ? ` · ${t("cardsDueDay")} ${cartao.dia_vencimento}` : ''}
            </p>
          </div>
          <span style={{
      ...
      <div style={{ display: 'grid', gap: 12, marginBottom: 24 }} className="grid grid-cols-2 md:grid-cols-4">
        {[
          { label: t("cardsInvoiceTotal"), valor: fatura.valor_total, cor: 'var(--of-text)' },
          { label: t("cardsSummaryPaid"), valor: fatura.valor_pago, cor: '#16A34A' },
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  ...
                  <span style={{
                    ...
                    background: tx.confirmado ? 'var(--of-success-bg)' : 'var(--of-warning-bg)',
                    color: tx.confirmado ? 'var(--of-success-text)' : 'var(--of-warning-text)',
                  }}>
                    {tx.confirmado ? `✓ ${t("cardsConfirmed")}` : t("cardsConfirm")}
                  </span>
                </div>
              </div>
            ))}
      ...
      {fatura.status !== 'paga' && fatura.valor_total > 0 && (
        <div style={{
          ...
          >
            {t("cardsPayInvoice")} · {valorFmt(saldo)}
          </button>
        </div>
      )}
