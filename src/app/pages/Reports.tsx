...
function generatePDF(
  ...
  const summaryItems = [
    { label: t("reportsTotalExpenses"), value: formatCurrency(totais.totalGastos), r: 211, g: 47, b: 47 },
    { label: t("reportsTotalIncome"), value: formatCurrency(totais.totalRenda), r: 56, g: 142, b: 60 },
    { label: "Saldo Disponivel", value: formatCurrency(totais.saldoDisponivel), r: 0, g: 0, b: 0 },
    { label: t("reportsSavings"), value: `${totais.percentualEconomia.toFixed(1)}%`, r: 0, g: 0, b: 0 },
  ];
...
export default function Reports() {
  ...
  if (!hasAccess) return <FeatureLock messageKey="featureLockReports" />;
...
function ReportsContent() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-[1000px] mx-auto">
      {/* Controles */}
      <div
        className="bg-card rounded-lg border border-border p-5 mb-5"
        style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}
      >
        <h3 className="text-foreground mb-4 flex items-center gap-2" style={{ fontSize: 14, fontWeight: 600 }}>
          <Calendar size={16} />
          {t("reportsConfigure")}
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="text-muted-foreground uppercase tracking-wider block mb-2" style={{ fontSize: 11, fontWeight: 500 }}>
              {t("reportsMonth")}
            </label>
            <select
              value={selectedMonth}
              onChange={(e) => setSelectedMonth(e.target.value)}
              className="w-full border border-border rounded-md px-3 py-2.5 outline-none focus:border-foreground transition-colors text-foreground bg-card"
              style={{ fontSize: 13 }}
            >
        ...
        <button
          ...
          className="mt-5 w-full sm:w-auto px-8 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors disabled:bg-muted disabled:text-muted-foreground flex items-center gap-2"
      ...
      >
        ...
        <div className="bg-primary text-primary-foreground px-6 py-5 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="w-6 h-6 bg-card rounded flex items-center justify-center">
                <span className="text-foreground" style={{ fontSize: 9, fontWeight: 700 }}>OF</span>
              </div>
              <span className="text-primary-foreground/80 uppercase tracking-wider" style={{ fontSize: 12, fontWeight: 600 }}>
                Openfy
              </span>
            </div>
            <h2 style={{ fontSize: 18, fontWeight: 700 }}>
              {t("reportsMonthly")} — {selectedMonth} {selectedYear}
            </h2>
          ...
          <div className="text-right">
            <p className="text-[#BBBBBB]" style={{ fontSize: 11 }}>{t("reportsGeneratedOn")}</p>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="px-6 py-5 border-b border-border">
              <h3 className="text-muted-foreground uppercase tracking-wider mb-4" style={{ fontSize: 13, fontWeight: 600 }}>
                {t("reportsExecutiveSummary")}
              </h3>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  { label: t("reportsTotalExpenses"), value: formatCurrency(totais.totalGastos), icon: ArrowDownRight, color: "#D32F2F" },
                  { label: t("reportsTotalIncome"), value: formatCurrency(totais.totalRenda), icon: ArrowUpRight, color: "#388E3C" },
                  { label: t("reportsBalance"), value: formatCurrency(totais.saldoDisponivel), icon: Wallet, color: "var(--of-text)" },
                  { label: t("reportsSavings"), value: `${totais.percentualEconomia.toFixed(1)}%`, icon: PiggyBank, color: "var(--of-text)" },
                ].map(({ label, value, icon: Icon, color }) => (
                  <div key={label} className="border border-border rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <Icon size={14} style={{ color }} />
                      <p className="text-muted-foreground" style={{ fontSize: 11, fontWeight: 500 }}>{label}</p>
                    </div>
            ...
            {categorias.length > 0 && (
              <div className="px-6 py-5 border-b border-border">
                <h3 className="text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2" style={{ fontSize: 13, fontWeight: 600 }}>
                  <BarChart2 size={14} />
                  {t("reportsByCategory")}
                </h3>
                  ...
                  {categorias.map((item) => (
                    <div key={item.name} className="flex items-center gap-4">
                      <div className="w-28 flex-shrink-0">
                        <p className="text-foreground" style={{ fontSize: 12, fontWeight: 500 }}>{item.name}</p>
                      </div>
                      <div className="flex-1 h-6 bg-muted rounded-full overflow-hidden">
                        <div
                          className="h-full bg-primary rounded-full flex items-center pl-3"
                          style={{ width: `${item.percent}%`, minWidth: "2rem" }}
                        >
                          <span className="text-primary-foreground" style={{ fontSize: 10, fontWeight: 600 }}>
                            {item.percent}%
                          </span>
                        </div>
                      </div>
            ...
            <div className="px-6 py-5 border-b border-border">
              <h3 className="text-muted-foreground uppercase tracking-wider mb-4" style={{ fontSize: 13, fontWeight: 600 }}>
                {t("reportsDetailedTransactions")} ({transacoes.length})
              </h3>
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    {[t("reportsDate"), t("reportsDescription"), t("reportsCategory"), t("reportsType"), t("reportsValue")].map((h, i) => (
                      <th
                        key={h}
                        className={`pb-2 text-muted-foreground ${i >= 3 ? "text-right" : "text-left"}`}
                ...
                <tbody>
                  {transacoes.map((tx, i) => (
                    <tr key={tx.id} className={i % 2 !== 0 ? "bg-muted/40" : ""}>
                      <td className="py-2.5 text-muted-foreground" style={{ fontSize: 12 }}>
                        {new Date(tx.data + "T00:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })}
                      </td>
                      <td className="py-2.5 text-foreground" style={{ fontSize: 12, fontWeight: 500 }}>
                        {tx.descricao || "—"}
                      </td>
                      <td className="py-2.5">
                        <span
                          className="px-2 py-0.5 rounded"
                          style={{ fontSize: 10, fontWeight: 500, backgroundColor: "var(--of-page-bg)", color: "var(--of-text-secondary)" }}
                        >
                          {tx.categorias?.nome || "Outros"}
                        </span>
                      </td>
                      <td className="py-2.5 text-right">
                        <span
                          className="px-2 py-0.5 rounded text-primary-foreground"
                          style={{
                            fontSize: 10,
                            fontWeight: 500,
                        ...
                        >
                          {tx.tipo === "despesa" ? t("modalDespesa") : t("modalReceita")}
                        </span>
                      </td>
                      <td
        ...
        <div className="px-6 py-4 bg-muted flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 bg-primary rounded flex items-center justify-center">
              <span className="text-primary-foreground" style={{ fontSize: 8, fontWeight: 700 }}>OF</span>
            </div>
            <span className="text-muted-foreground" style={{ fontSize: 11 }}>Openfy — Gestão Financeira</span>
          </div>
          <span className="text-[#999999]" style={{ fontSize: 11 }}>
            {t("reportsConfidential")} · {new Date().getFullYear()}
          </span>
        </div>
      </div>
    </div>
