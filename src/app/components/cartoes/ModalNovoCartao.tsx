...
export default function ModalNovoCartao({ userId, onClose, onSalvo }: Props) {
  ...
  const validar = () => {
    ...
    if (!nome.trim()) e.nome = t("cardsCardNameRequired")
    if (tipo === 'credito') {
      const dia = parseInt(diaVencimento)
      const fech = parseInt(diaFechamento)
      if (diaVencimento && (dia < 1 || dia > 31)) e.diaVencimento = t("cardsInvalidDay")
      if (diaFechamento && (fech < 1 || fech > 31)) e.diaFechamento = t("cardsInvalidDay")
    }
    return e
  }
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      toast.success(t("cardsAddedSuccess"))
      onSalvo(cartao)
    } catch {
      toast.error(t("cardsSaveError"))
    } finally {
      setSalvando(false)
    }
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("cardsCardNamePlaceholder")}
              style={{
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {([['credito', t("cardsCredit"), CreditCard], ['debito', t("cardsDebit"), Building2]] as const).map(([val, label, Icon]) => (
                <button
                  key={val}
                  type="button"
                  onClick={() => setTipo(val)}
              ...
              {CORES_PRESET.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    background: c.valor,
                    border: cor === c.valor ? '3px solid var(--of-text)' : '2px solid transparent',
                    outline: cor === c.valor ? '2px solid var(--of-surface)' : 'none',
                    outlineOffset: -4,
                    cursor: 'pointer',
          ...
          >
            ...
            <span style={{
              ...
              {tipo === 'credito' ? t("cardsCredit") : t("cardsDebit")}
            </span>
          </div>
